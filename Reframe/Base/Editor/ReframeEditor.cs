	...
	public class ReframeEditor : Editor {
		...
		protected static bool ValidateCreateObj(MenuCommand menuCommand, string name) {
			...
			if (contextGO.GetComponent<VRCAvatarDescriptor>() == null)
				return false;
			if (contextGO.transform.parent != null && contextGO.transform.parent.GetComponent<VRCAvatarDescriptor>() != null)
				return false;
		...
		protected static void CreateObj<T>(MenuCommand menuCommand, string name)
			...
			go.AddComponent<T>();
			go.AddComponent<EditOutfitRuntime>();
		...
		protected void PropTransform(PhysBoneInfo physBoneInfo) {
			...
			if (prop != null) {
				EditorGUILayout.PropertyField(
					prop,
					new GUIContent($"{physBoneInfo.name}【PB : {physBoneInfo.PBCount},Transform : {physBoneInfo.TransformCount}】")
				);
			}
		}

		protected SerializedProperty GetSerializedProperty(string nameFlg) {
			var fi = GetType().GetField(nameFlg, BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public);
		...
		protected void PropColliders(PhysBoneInfo info, params (string title, string flgName, float countWeight)[] titlesAndNames) {
			...
			List<SerializedProperty> PropCollider(string title, string flgName, float countWeight) {
				...
				if (prop != null) {
					props.Add(prop);
					EditorGUILayout.PropertyField(prop, new GUIContent($"{title} : {info.ColliderCount * countWeight}"));
				}
				return props;
			}
		...
		protected void ExecuteMode() {
			...
			executeMode.enumValueIndex = EditorGUILayout.Popup("FX、EXメニュー編集動作モード", selected, new[] { "事前実行", "ビルド時" });
#else
			selected = 0;
			EditorGUILayout.HelpBox("NDMFがインストールされている場合のみ実行モードの変更が有効になります。", MessageType.Info);
#endif
		}
		...
		protected void DelMenu(SerializedProperty textureResize, SerializedProperty AAORemoveFlg) {
			...
			textureResize.enumValueIndex = EditorGUILayout.Popup("メニュー画像解像度設定", selected, new[] { "下げる", "削除" });

#if !AVATAR_OPTIMIZER_FOUND
			GUI.enabled = false;
			EditorGUILayout.HelpBox("AAOがインストールされている場合のみ「頬染めを削除」が有効になります。", MessageType.Info);
#endif
		...
		protected void QuestDialog(ReframeRuntime target, SerializedProperty questFlg1, string questHelp) {
			...
			if (target.transform.root.GetComponent<TraceAndOptimize>() == null)
				EditorGUILayout.HelpBox("アバターにTraceAndOptimizeを追加してください", MessageType.Error);
#else
			EditorGUILayout.HelpBox("AvatarOptimizerが見つかりませんVCCに追加して有効化してください", MessageType.Error);
#endif
		...
		protected void Count(List<PhysBoneInfo> PhysBoneInfoList, int pbCount, int pbTCount, int pbCCount) {
			int count = pbCount;
			int count1 = pbTCount;
			int count2 = pbCCount;
