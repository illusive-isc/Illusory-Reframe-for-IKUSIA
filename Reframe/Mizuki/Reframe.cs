					...
					if (!(TPSFlg && ClairvoyanceFlg))
						foreach (var state in layer.stateMachine.states) {
							if (state.state.motion is BlendTree blendTree) {
								...
								{
									new() {
										motion = AssetDatabase.LoadAssetAtPath<Motion>(AssetDatabase.GUIDToAssetPath( "f9530a93f4a5d4845b11eb2e5cd883f8")
										),
										threshold = 0.0f,
										timeScale = 1,
									},
									new() {
										motion = AssetDatabase.LoadAssetAtPath<Motion>(AssetDatabase.GUIDToAssetPath("0a648115c24411e45a2ff52191e987be")),
										threshold = 1.0f,
										timeScale = 1,
									},
								};
