...
namespace jp.illusive_isc.IllusoryReframe.IKUSIA.Rurune {
	internal class Hair : Base {
		...
		internal override void ChangeObj(params string[] delPath) {
			RuruneReframe reframe = (RuruneReframe)this.reframe;
			...
			if (reframe.HairFlg50)
				base.ChangeObj("Armature/Hips/Spine/Chest/Neck/Head/rurune_headphone");
			else
				avatarRoot
					.Find("Armature/Hips/Spine/Chest/Neck/Head/rurune_headphone")
		...
		private static void SetHairPB(RuruneReframe reframe, UnityEngine.Transform hairRoot) {
			if (hairRoot) {
				if (
